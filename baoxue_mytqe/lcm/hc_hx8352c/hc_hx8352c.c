...
static void lcm_init(void)
{
	...
	#ifdef READ_CC
	lcd_id = get_lcd_id_n(0xCC, 2);
	#else
	lcd_id = get_lcd_id_n(0xEF, 4);
	lcd_id = (lcd_id << 8) | (get_lcd_id_n(0xEF, 5) & 0xff);
	#endif
	LCD_PRINT("[HC_HX8352C]lcm_init: lcd_id = 0x%x\n", lcd_id);
...
static void lcm_resume(void)
{
	...
	LCD_PRINT("[HC_HX8352C]lcm_resume: lcd_id = 0x%x\n", lcd_id);
...
static unsigned int get_lcd_id_r(unsigned int addr)
{
	...
	LCD_PRINT("[HC_HX8352C]get_lcd_id_r: id = 0x%x\n", id);
	return id;
}
...
static unsigned int get_lcd_id_n(unsigned int addr, unsigned char count)
{
	...
	LCD_PRINT("[HC_HX8352C]get_lcd_id_n: id = 0x%x\n", id);
...
{	
	...
	LCD_PRINT("[HC_HX8352C]lcm_compare_id: lcd_id = 0x%x\n", lcd_id);
