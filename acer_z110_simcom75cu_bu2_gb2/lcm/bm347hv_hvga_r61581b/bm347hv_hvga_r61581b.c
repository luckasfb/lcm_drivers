...
static __inline unsigned char spi_get_3bytes(unsigned int reg)
{
	unsigned int i;
...
{
#ifdef BUILD_UBOOT
	printf("init_lcm_registers --r61581bbbbb---3579ti \r\n");
#else
	...
	send_ctrl_cmd(0xC6);
	send_data_cmd(0x02);//rising edge
	...
	send_data_cmd(0x07);
	send_data_cmd(0x07);
	send_data_cmd(0x14);//1c//18
	send_data_cmd(0x03);

	send_ctrl_cmd(0xD1);
	send_data_cmd(0x03);
	send_data_cmd(0x57);//VCM
...
static unsigned int lcm_compare_id(void)
{
	...
	id=spi_get_3bytes(0xbf);
	

#ifdef BUILD_UBOOT
	printf("RQ______dali22 __3579ti__lcd_id=%x  \r\n",id);
#else
...
//	SET_RESET_PIN(1);
  //     MDELAY(30);
    return 1;//(LCM_ID == id)?1:0;  // LCM_ID=0x46
...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dpi.hsync_back_porch  = 20;		//punk 8 20
	params->dpi.hsync_front_porch = 10;		//punk 8 20
	...
	params->dpi.io_driving_current = LCM_DRIVING_CURRENT_4MA;//LCM_DRIVING_CURRENT_6575_4MA;
...
// ---------------------------------------------------------------------------
LCM_DRIVER bm347hv_hvga_r61581b_lcm_drv = 
{
    .name			= "bm347hv",
