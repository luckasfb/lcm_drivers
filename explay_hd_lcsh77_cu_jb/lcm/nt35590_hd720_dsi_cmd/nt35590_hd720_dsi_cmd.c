...
//#define __NT35590_HIGH_SPEED__

//#define MTK_REGISTER_SETTING
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		#ifdef __NT35590_HIGH_SPEED__
		params->dsi.pll_div1=37;     //26;		// fref=26MHz, fvco=fref*(div1+1)	(div1=0~63, fvco=500MHZ~1GHz)
		#else
...
#if 0   //error report
extern unsigned char err_report_buf[8];
static unsigned int lcm_esd_check(void)
{
    ...
    array[0] = 0x00013700;//0AH only 1 para
    ...
    #if 0   //ndef BUILD_UBOOT
    for(i = 0; i < 7; i++)                
      printk(KERN_EMERG "++++err_report_buf[%d]:0x%x \n",i,err_report_buf[i]);
    #endif

    if(err_report_buf[1] == 0x9C)
    {
        if(err_report_buf[4] == 0x02 && err_report_buf[5] == 0x02)
        {
            //fResult = TRUE;
            err_count ++;
        }
      ...
      //printk(KERN_EMERG "++++esd_check, err_count=%d, need rst=%d\n",err_count,  fResult);
...
#endif /* (!defined(BUILD_UBOOT) && !defined(BUILD_LK)) */
}    
#else
static unsigned int lcm_esd_check(void)
{
...
static unsigned int lcm_compare_id(void)
{
	...
	id1 = read_reg(0xDA);	// the display module's manufacturer
	id2 = read_reg(0xDB);	// the display module/driver version
	id3 = read_reg(0xDC);	// the display module/driver
	id4 = read_reg(0xF4);	// the Novatek ID code

	// Set Maximum return byte = 3
	array[0] = 0x00033700;
	dsi_set_cmdq(array, 1, 1);
	read_reg_v2(0x04, buffer, 3);
	/*
		buffer[0] - LCD module's manufacturer ID
		buffer[1] - LCD module/driver version ID
		buffer[2] - LCD module/driver ID
	*/

#if (!defined(BUILD_UBOOT) && !defined(BUILD_LK))
	printk("%s, Module ID = {%x, %x, %x, %x} \n", __func__, id1, id2, id3, id4);
	printk("%s, Display ID = {%x, %x, %x, %x} \n", __func__, buffer[0], buffer[1], buffer[2]);
#else
	printf("%s, Module ID = {%x, %x, %x, %x} \n", __func__, id1, id2, id3, id4);
	printf("%s, Display ID = {%x, %x, %x, %x} \n", __func__, buffer[0], buffer[1], buffer[2]);
#endif

    return 1;//(LCM_ID == id4)?1:0;
}
